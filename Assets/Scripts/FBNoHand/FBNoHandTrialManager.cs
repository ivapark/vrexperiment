...
public class FBNoHandTrialManager : MonoBehaviour
{
    public static FBNoHandTrialManager Instance;

    public int currentTrial = 0;
    public int repetitionsPerCondition = 4; // Each condition repeated 4 times
    private List<FBNoHandTrialCondition> allTrials;
    ...
    void GenerateTrials()
    {
        allTrials = new List<FBNoHandTrialCondition>();
        ...
        for (int t = 0; t < targetCount; t++)
        {
            for (int p = 0; p < penaltyCount; p++)
            {
                ...
                {
                    allTrials.Add(new FBNoHandTrialCondition(t, p, conditionIndex));
                }
                conditionIndex++;
            }
        }
    ...
    /// </summary>
    public FBNoHandTrialCondition GetNextTrial()
    {
    ...
    /// </summary>
    public void ReinsertTrial(FBNoHandTrialCondition failedTrial)
    {
...
[System.Serializable]
public class FBNoHandTrialCondition
{
    public int targetIndex;
    public int penaltyIndex;
    public int conditionIndex; // unique condition number (0-26)

    public FBNoHandTrialCondition(int t, int p, int c)
    {
        targetIndex = t;
        penaltyIndex = p;
        conditionIndex = c;
