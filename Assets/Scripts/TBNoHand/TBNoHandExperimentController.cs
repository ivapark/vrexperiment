...
public class TBNoHandExperimentController : MonoBehaviour
{
    public Transform rightHandController;
    public MovementLogger dataLogger;
    public TargetController targetController;
    public TBNoHandPenaltyController PenaltyController;
    public RightHandVisualState visualState;
    ...
    void Start()
    {
        ...
        Debug.Log($"[TBExperimentController] Using Target Seed: {targetSeed}");
    ...
    IEnumerator RunTrials()
    {
        ...
        while (true)
        {
            TBNoHandTrialCondition condition = TBNoHandTrialManager.Instance.GetNextTrial();
            if (condition == null)
            {
            ...
            trialCount++;
            Debug.Log($"Starting Trial {trialCount}/{TBNoHandTrialManager.Instance.TotalTrialCount}");
            ...
            if (earlyExit && outcome == "Too Early")
            {
                ...
                TBNoHandTrialManager.Instance.ReinsertTrial(condition);
    ...
    {
        visualState?.SetStartPhase();  // VISUAL TOGGLE
            ...
            else
            {
                if (velocity.magnitude < velocityThreshold)
                {
                    ...
                    if (stillTimer >= requiredStillTime)
                    {
                        ...
                        visualState?.SetGoPhase();  // beam OFF at GO
    ...
    )
    {
        ...
        while (!pointEvaluated)
        {
            ...
            if (movementStarted)
            {
                ...
                // Stable end rule: speed + accel both small for a duration
                ...
                {
                    endPos = currentPos;   // endpoint = final stabilized point
                    endTime = Time.time;
    ...
    private void ShowEndpointMarker(Vector3 position)
    {
        ...
        {
            Debug.LogWarning("[TBExperimentController] No endpointMarkerPrefab assigned.");
            return;
        }
