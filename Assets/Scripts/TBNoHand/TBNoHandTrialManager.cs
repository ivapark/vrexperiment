...
public class TBNoHandTrialManager : MonoBehaviour
{
    public static TBNoHandTrialManager Instance;

    public int currentTrial = 0;
    public int repetitionsPerCondition = 4; // Each condition repeated 4 times
    private List<TBNoHandTrialCondition> allTrials;
    ...
    void GenerateTrials()
    {
        allTrials = new List<TBNoHandTrialCondition>();
        ...
        for (int t = 0; t < targetCount; t++)
        {
            for (int p = 0; p < penaltyCount; p++) // 0: none, 1: top, 2: bottom
            {
                ...
                {
                    allTrials.Add(new TBNoHandTrialCondition(t, p, conditionIndex));
                }
                conditionIndex++;
            }
        }
    ...
    /// </summary>
    public TBNoHandTrialCondition GetNextTrial()
    {
    ...
    /// </summary>
    public void ReinsertTrial(TBNoHandTrialCondition failedTrial)
    {
...
[System.Serializable]
public class TBNoHandTrialCondition
{
    public int targetIndex;
    public int penaltyIndex;
    public int conditionIndex; // unique condition number (0–26)

    public TBNoHandTrialCondition(int t, int p, int c)
    {
        targetIndex = t;
        penaltyIndex = p;
        conditionIndex = c;
