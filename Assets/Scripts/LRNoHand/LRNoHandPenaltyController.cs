...
public class LRNoHandPenaltyController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log($"[PenaltyControllerLR] BasePenaltySeed = {basePenaltySeed}");
    }

    /// <summary>
    /// Applies a penalty zone based on penaltyIndex. If penaltyIndex == 0, no penalty is applied.
    /// </summary>
    public void ApplyPenalty(GameObject target, int penaltyIndex)
    {
        ...
        if (penaltyIndex == 0)
        {
            Debug.Log("[PenaltyControllerLR] No penalty applied for this trial.");
            ClearPenalty();
            usedSeedPenalty = -1;
        ...
        // Directions: 1=left, 2=right, etc. Adjust based on index mapping.
        ...
        Debug.Log($"[PenaltyControllerLR] PenaltySeed={usedSeedPenalty}, First={firstSphere}, Last={lastSphere}");
    }
