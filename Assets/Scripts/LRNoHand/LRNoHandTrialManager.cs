...
public class LRNoHandTrialManager : MonoBehaviour
{
    public static LRNoHandTrialManager Instance;

    public int currentTrial = 0;
    public int repetitionsPerCondition = 4; // Each condition repeated 4 times
    private List<LRNoHandTrialCondition> allTrials;
    ...
    void GenerateTrials()
    {
        allTrials = new List<LRNoHandTrialCondition>();
        ...
        for (int t = 0; t < targetCount; t++)
        {
            for (int p = 0; p < penaltyCount; p++)
            {
                ...
                {
                    allTrials.Add(new LRNoHandTrialCondition(t, p, conditionIndex));
                }
                conditionIndex++;
            }
        }
    ...
    /// </summary>
    public LRNoHandTrialCondition GetNextTrial()
    {
    ...
    /// </summary>
    public void ReinsertTrial(LRNoHandTrialCondition failedTrial)
    {
...
[System.Serializable]
public class LRNoHandTrialCondition
{
    public int targetIndex;
    public int penaltyIndex;
    public int conditionIndex; // unique condition number (0-26)

    public LRNoHandTrialCondition(int t, int p, int c)
    {
        targetIndex = t;
        penaltyIndex = p;
        conditionIndex = c;
