...
public class TargetOnlyTargetController : MonoBehaviour
{
    ...
    public float targetRadius = 0.03f;      // to make it consistent with TargetController.cs
    ...
    void CreateDots(GameObject targetGroup, Vector3 pos)
    {
        ...
        for (int i = 0; i < 250; i++)
        {
            //Vector3 offset = Random.insideUnitSphere * 0.03f; //Iva's old version: radius 3cm
            Vector3 offset = Random.insideUnitSphere * targetRadius; // use value from Inspector
