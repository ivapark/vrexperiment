...
public class TBExperimentController : MonoBehaviour
{
    public Transform rightHandController;
    public MovementLogger dataLogger;
    public TargetController targetController;
    public TBPenaltyController PenaltyController;
    ...
    {
        // Wait for targets to finish generating
    ...
    IEnumerator RunTrials()
    {
        ...
        {
            // Fetch next trial
            TBTrialCondition27 condition = TBTrialManager27.Instance.GetNextTrial();
            if (condition == null)
            {
                Debug.Log("All trials complete.");
                ShowFinalScore();
                // instead of showing final score it could display leaderboard like LR if you want:
            ...
            trialCount++;
            Debug.Log($"[TB] Starting Trial {trialCount}/{TBTrialManager27.Instance.TotalTrialCount}");
            ...
            if (earlyExit && outcome == "Too Early")
            {
                ...
                TBTrialManager27.Instance.ReinsertTrial(condition);
    ...
    private void SetFeedback(string outcome, GameObject target, bool hasPenalty)
    {
        ...
        // --- AUDIO FEEDBACK (same pattern as LR) ---
        if (audioSource != null)
        {
            switch (outcome)
            {
    ...
    private void ShowFinalScore()
    {
        ...
        Debug.Log($"[TB Feedback] Final Score: {finalScore}");
