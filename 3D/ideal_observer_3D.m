...
% ----------------------------------------------------------------------
% Input(s) :
% empirical_cov_results.mat (obtained from Luhe's code)
% 'SigmaByCond'
% 'NByCond'
% ----------------------------------------------------------------------
% Output(s):
...
baseResultsDir = fullfile(pwd, 'Results', subID, 'Model_3D');
baseFigDir = fullfile(pwd, 'Figures', subID, 'Model_3D');
...
loadFileName = sprintf('%s_empirical_cov_results.mat', subID);
fullLoadPath = fullfile(targetPath, loadFileName);
load(fullLoadPath, 'SigmaByCond', 'NByCond');
...
% 1D: searches along penalty axis only
% 3D: dx, dy, dz
...
for pp = 1:n_penalty_cond
    ...
    for ii = 1:n_target
        ...
        % BADS starting points
        best_shift_3d = [0, 0, 0];
        best_neg_eg = inf;

        % Generate grid-based starting points using Luhe's getInit
        num_sections = 2 * n_bads_runs;
        start_points = getInit(lb, ub, num_sections, n_bads_runs);
...
for pp = 1:n_penalty_cond
    ...
    b(1).FaceColor = [41,157,143]./255;   % X
    b(2).FaceColor = [233,196,106]./255;  % Y
    b(3).FaceColor = [106, 92, 164]./255;  % Z
...
subjectFigFolder = fullfile(baseFigDir);
if ~exist(subjectFigFolder, 'dir')
    mkdir(subjectFigFolder);
end

saveFileName = sprintf('%s_ideal_3D_shift_components.png', subID);
...
% Notes:
% How much the 3D optimal aimpoint deviates from the penalty axis.
% If off-diagonal covariance has no effect (i.e., x,y,z independent),
% |off-axis shift| should be ~0...
...
for pp = 1:n_penalty_cond
    ...
    other_axes = setdiff(1:3, p_axis); % other axes
...
saveFileName_2 = sprintf('%s_ideal_3D_off_axis_shift.png', subID);
...
saveFileName = sprintf('%s_optimal_observer_3D_results.mat', subID);

subjectFolder = fullfile(baseResultsDir);
if ~exist(subjectFolder, 'dir'), mkdir(subjectFolder); end
fullSavePath = fullfile(subjectFolder, saveFileName);
save(fullSavePath, 'results');
fprintf('All saved!\n');
