...
baseResultsDir_1D = fullfile(pwd, 'Results', subID, 'Model_1D');
baseFigDir_1D = fullfile(pwd, 'Figures', subID, 'Model_1D');
...
if plt_EG_landscape == 1
    ...
    subjectFigFolder = fullfile(baseFigDir_1D);
    if ~exist(subjectFigFolder, 'dir')
        mkdir(subjectFigFolder);
    end

    saveFileName = sprintf('%s_ideal_1D_shift_vs_sigma.png', subID);
    ...
    saveFileName_2 = sprintf('%s_ideal_1D_eg_landscape.png', subID);
    fullSavePath_2 = fullfile(subjectFigFolder, saveFileName_2);
    saveas(gcf, fullSavePath_2);

    %% ========== PLOT 3: All targets, one figure per penalty direction =======

    target_order_plotting = [7,8,9,4,5,6,1,2,3];
    ...
    for pp = 1:n_penalty_cond
        ...
        for subplot_idx = 1:9
            ii = target_order_plotting(subplot_idx);
        ...
        saveFileName_3 = sprintf('%s_ideal_1D_eg_all_targets_%s.png', subID, penalty_labels{pp});
        fullSavePath_3 = fullfile(subjectFigFolder, saveFileName_3);
        saveas(gcf, fullSavePath_3);


    end
end % plt_EG_landscape
...
saveFileName_1D = sprintf('%s_optimal_observer_1D_results.mat', subID);
subjectFolder = fullfile(baseResultsDir_1D);
if ~exist(subjectFolder, 'dir'), mkdir(subjectFolder); end
fullSavePath = fullfile(subjectFolder, saveFileName_1D);
