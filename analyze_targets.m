...
for ii = 1:length(target_id)
    ...
    target_coor = [this_target_trials.TargetX, this_target_trials.TargetY, this_target_trials.TargetZ];
    target_coor = unique(target_coor, 'rows');
    TARGET{ii} = target_coor;

    % Data of these 
    endpoint = [this_target_trials.EndX, this_target_trials.EndY, this_target_trials.EndZ];
...
for ii = 1:length(target_id)
    ...
    % % Plot the endpoints
     ...
     mu = [TARGET{ii}(:,1), TARGET{ii}(:,2), TARGET{ii}(:,3)]';
     C = COV{ii};
