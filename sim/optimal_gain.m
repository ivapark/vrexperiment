...
if strcmp(model.mode, 'initialize')
    ...
    z = free_param(3);
        ...
        % sample endpoints
        endpoint = mvnrnd([x, y, z], model.empirical_cov, model.sim_trial);
        
        % for each endpoint, calculate the gain function
        out = gain_function(model.target_coor, model.penalty_coor, endpoint, model);

end

end

function gain = gain_function(target_coor, penalty_coor, endpoint, model)
    ...
    penalty_dim = (penalty_coor - target_coor)~=0;
    penalty_distance = target_coor(penalty_dim) - penalty_coor(penalty_dim);

    if penalty_distance > 0 % e.g., penalty is on the right of the target
        penalty_trial = squeeze(endpoint(penalty_dim) - penalty_coor(penalty_dim))>0;
        gain(penalty_trial) = model.penalty_gain;
